...
Internal | LAN/VPN only — the domain resolves only via AdGuard split-horizon, no public DNS record / Caddy refuses non-private source ranges
...
Service Name | Internal Port | Domain | Exposure | Auth Provider | Notes
--- | --- | --- | --- | --- | ---
Ollama | 11434 | api.{{ homelab_domain }} | Internal | - | Local LLM backend. Forward-auth breaks API clients anyway; reach it over VPN
