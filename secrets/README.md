...
All files here are SOPS-encrypted with age. See `/.sops.yaml`. Nothing in this directory is ever committed in plaintext, because the repo is treated as public.
...
- Docker Compose: render an env file at deploy time, e.g. `sops decrypt secrets/edge-caddy.sops.env > apps/edge/caddy/.env` (the `.env` is gitignored)
...
| `environment.sops.yaml` | Environment-specific values such as domains, ACME email, offsite repo URL, and VPS addresses |
| `edge-adguard.sops.yaml` | `adguard_password_hash` injected into `AdGuardHome.yaml` at deploy |
| `edge-authentik.sops.env` | `PG_PASS` + `AUTHENTIK_SECRET_KEY` for the Authentik stack |
| `edge-backup.sops.yaml` | Restic password for the local edge backup repository, plus OVH S3 key/password for offsite edge backup |
